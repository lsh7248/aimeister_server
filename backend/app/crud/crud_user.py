...
class CRUDUser(CRUDBase[User, RegisterUserParam, UpdateUserParam]):
    ...
    async def update_userinfo(
        self, db: AsyncSession, input_user: User, obj: UpdateUserParam
    ) -> int:
        user = await db.execute(
            update(self.model)
            .where(self.model.id == input_user.id)
    ...
    async def update_avatar(
        self, db: AsyncSession, current_user: User, avatar: AvatarParam
    ) -> int:
        user = await db.execute(
            update(self.model)
    ...
    async def reset_password(
        self, db: AsyncSession, pk: int, password: str, salt: str
    ) -> int:
        user = await db.execute(
            update(self.model)
            .where(self.model.id == pk)
            .values(password=await jwt.get_hash_password(password + salt))
        )
        return user.rowcount
    ...
    async def set_staff(self, db: AsyncSession, user_id: int) -> int:
        ...
        user = await db.execute(
            update(self.model)
            .where(self.model.id == user_id)
            .values(is_staff=False if staff_status else True)
        )
        return user.rowcount
    ...
    async def set_status(self, db: AsyncSession, user_id: int) -> int:
        ...
        user = await db.execute(
            update(self.model)
            .where(self.model.id == user_id)
            .values(status=False if status else True)
        )
        return user.rowcount
    ...
    async def set_multi_login(self, db: AsyncSession, user_id: int) -> int:
        ...
        user = await db.execute(
            update(self.model)
            .where(self.model.id == user_id)
            .values(is_multi_login=False if multi_login else True)
        )
        return user.rowcount
