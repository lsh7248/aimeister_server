    ...
    async def swagger_login(
        ...
        async with async_db_session() as db:
            ...
            if not current_user:
                ...
                form_data.password + current_user.salt, current_user.password
            ):
                raise errors.AuthorizationError(msg="잘못된 비밀번호입니다.")
            elif not current_user.status:
                raise errors.AuthorizationError(
                    msg="사용자가 잠겨 있습니다. 로그인에 실패하였습니다."
                )
            # 로그인 시간 업데이트
        ...
        async with async_db_session() as db:
            try:
                ...
                refresh_token, refresh_token_expire_time = (
                    await jwt.create_refresh_token(
                        str(user.id),
    ...
    async def new_token(
        *, request: Request, refresh_token: str
        ...
        if request.user.id != user_id:
            raise errors.TokenError(msg="새로 고침 토큰이 유효하지 않습니다.")
        async with async_db_session() as db:
